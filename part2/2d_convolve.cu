...
//bounds checker
__device__ int gpu_clamp(int x, int lo, int hi) {
    if (x < lo)
        return lo;
    else if (x > hi)
        return hi;
...
//kernel
__global__ void GPU_convolve_kernel1(float *matrix, float *out, float *filter, int N, int K, int max, int min) {    
    int block_id = blockIdx.x + gridDim.x * blockIdx.y;
    int thread_id = blockDim.x * block_id + threadIdx.x; // ID of individual pixel

    // setup
    float result = 0.0;
    float *center = &filter[K * K / 2];
    int row = thread_id / N;
    int col = thread_id % N;
    ...
    for (int i = min; i <= max; ++i) {
        for (int j = min; j <= max; ++j) {
            result += matrix[gpu_clamp(row + i, 0, N-1)*N + gpu_clamp(col + j, 0, N-1)] * center[i * K + j];
        }
    }
...
void GPU_convolve(float *A_in, float *A_out, int N, float *F, int K, int kernel_code, float *kernel_runtime, float *transfer_runtime) {
    ...
    int vector_size = N * N * sizeof(float);
    int filter_size = K * K * sizeof(float);

    // Allocate GPU memory for the inputs and results
    float *in_GPU, *out_GPU, *F_GPU;
    ...
    if (cudaMalloc((void **) &F_GPU, filter_size) != cudaSuccess) die("Error allocating GPU memory");
    
    // Transfer the input vectors to GPU memory
    cudaMemcpy(in_GPU, A_in, vector_size, cudaMemcpyHostToDevice);
    cudaMemcpy(F_GPU, F, filter_size, cudaMemcpyHostToDevice);    
    ...
    // Execute the kernel to compute the 2d Convolution
    ...
    switch(kernel_code){	
    case 1 :
	GPU_convolve_kernel1 <<< grid_size, threads_per_block >>> (in_GPU, out_GPU, F_GPU, N, K, max_offset, min_offset);
	break;
    case 2 :
    	die("NO KERNEL\n");
	break;
    ...
    //copy C back
    cudaMemcpy(A_out, out_GPU, vector_size, cudaMemcpyDeviceToHost);
    checkError();
    cudaDeviceSynchronize();  // this is only needed for timing purposes
