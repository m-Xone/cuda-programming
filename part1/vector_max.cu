...
//#################################################
// ################ SHARED MEMORY ################
__global__ void vector_max_kernel2(float *in_orig, float *out, int N) {
    int block_id = blockIdx.x + gridDim.x * blockIdx.y;
    int thread_id = blockDim.x * block_id + threadIdx.x;
    int id = threadIdx.x;
    __shared__ float in[256];
    in[id] = in_orig[thread_id];
    __syncthreads();
...
// ################ INTERLEAVING ################
__global__ void vector_max_kernel3(float *in_orig, float *out, int N) {
    int block_id = blockIdx.x + gridDim.x * blockIdx.y;
    int thread_id = blockDim.x * block_id + threadIdx.x;    
    int id = threadIdx.x;
    __shared__ float in[256];
    ...
    // EDGE CASE
    int end = threads_per_block;
    if(id == 0 && thread_id + threads_per_block > N) { end = N - thread_id; }    
    
    
    // LOOP
    // each thread runs a maximum of 8 loops, comparing itself against neighboring threads
    for(unsigned int n = 1; n < 256; n *= 2) {
	if(id % (2 * n) == 0)
            if((id + n < end) && (in[id] < in[id + n])) { in[id] = in[id + n]; }    
	//__syncthreads(); 
    }
    
    // BLOCK RESULT
    if(id == 0) { out[block_id] = in[0]; }
}
// ########## BRANCH DIVERGENCE REDUCTION ##########
__global__ void vector_max_kernel4(float *in_orig, float *out, int N) {
    int block_id = blockIdx.x + gridDim.x * blockIdx.y;
    int thread_id = blockDim.x * block_id + threadIdx.x;    
    int id = threadIdx.x;
    __shared__ float in[256];
    ...
    // LOOP
    // the first n/2 threads will do work (adjacent vs interleaved reduction) 
    // Reference: Nvidia's branch divergence optimization guide
    for(unsigned int n = 1; n < 256; n <<= 1) {
    	int m = id * (2 * n);
	if(m+n < end)
	    if(in[m] < in[m + n]) { in[m] = in[m + n]; }
	//__syncthreads(); 
    }
...
float GPU_vector_max(float *in_CPU, int N, int kernel_code, float *kernel_runtime, float *transfer_runtime) {
	...
	while(n > 1) {
		...
		n = (n>>8) + (1*((n % 256) && 1));
		cudaMemcpy(in_GPU, out_GPU, n*sizeof(float), cudaMemcpyDeviceToDevice);
    		int num_blocks = (int) ((float) (n + threads_per_block - 1) / (float) threads_per_block);
    		int max_blocks_per_dimension = 65535;
    		int num_blocks_y = (int) ((float) (num_blocks + max_blocks_per_dimension - 1) / (float) max_blocks_per_dimension);
    		int num_blocks_x = (int) ((float) (num_blocks + num_blocks_y - 1) / (float) num_blocks_y);
